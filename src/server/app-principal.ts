                ...
                if (rows && rows.length > 0) {
                    ...
                    var result2 = await client.query(`
                        insert into fichadas_recibidas(id_origen, fichador, dispositivo, fecha, hora, punto_gps,tipo)
                            SELECT id_origen, usuario, 'RECUPERACION_IW', momento::date, date_trunc('second', momento::time)::time, round(latitud,4)||','||round(longitud,4), tipo
...
export const getConfigFichadasDb = (be:AppBackend)=> ({
    ...
    requestTimeout: 5000
} as ConfigFichadasDb | {database?:null});
...
const cronSincroUsuarios = async (be: AppBackend) => {
    ...
    const interval = setInterval(async () => {
        ...
        if (configFichadasDb.database != null) try {
            await be.inTransaction(null, async (client) => {
