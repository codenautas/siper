...
function sectores_def(name:string, usuarioPuedeEditar: boolean, extendido: boolean): TableDefinition{
    return {
        ...
        fields: [
            {...sector, generatedAs:`coalesce(case when nivel<2 or substr(subsector,1,1) = 'X' then subsector else pertenece_a||rpad('',nivel-length(pertenece_a)-1,'0')||subsector end, anterior)`},
