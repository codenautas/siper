...
ADD COLUMN principal BOOLEAN DEFAULT true;

--idper repetidos, marcar solo 1 como principal
SELECT 
    idper, 
    COUNT(*) AS cantidad,
    ARRAY_AGG(usuario) AS ids_usuarios
FROM 
    siper.usuarios
WHERE 
    idper IS NOT NULL
...
alter table "cola_sincronizacion_usuarios_modulo" alter column "idper" set not null;
